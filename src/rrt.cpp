...
void RRT::odom_callback(const nav_msgs::Odometry::ConstPtr &odom_msg) {
    ...
    if(start_node.x<8.00 && start_node.y<1.50)
    {
    	goal_x= start_node.x+2.00;
    	goal_y= 0.00;	
    }
    else if(start_node.x>=8.00 && start_node.y<7.30)
    {
    	goal_x= 9.40;
    	goal_y= start_node.y+2.00;
    }
    else if(start_node.y>=7.30 && start_node.x>-12.0)
    {
    	goal_x= start_node.x-2.00;
    	goal_y= 8.70;
    } 
    ...
    for (int iter=0; iter<MAX_ITER; iter++){
        vector<double> sampled_point = sample(goal_x, goal_y);
        ...
        if(iter>MIN_ITER && !nodes_near_goal.empty()){
            ...
            for (int i=0; i<path_found.size(); i++){
                ...
                pp.pose.position.x = path_found.at(i).x;
                pp.pose.position.y = path_found.at(i).y;
            ...
            track_path(path);
            visualize_tree(tree, goal_x, goal_y);
            //ROS_INFO("path found");
            break;
        }
    }
...
std::vector<double> RRT::sample(double goal_x, double goal_y) {
   ...
   // double y = car_pose_.position.y + uni_dist_(gen_)*Y_SAMPLE_RANGE;
    std::normal_distribution<double> norm_dist_x(0.6*goal_x+0.4*car_pose_.position.x, STD);
    std::normal_distribution<double> norm_dist_y(0.6*goal_y+0.4*car_pose_.position.y, STD);
...
void RRT::visualize_tree(vector<Node>& tree, double goal_x, double goal_y){
    // plot goal first
