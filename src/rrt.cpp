...
void RRT::pose_callback(const geometry_msgs::PoseStamped::ConstPtr &pose_msg)
{
    ...
    */
    ...
    /*
    if(current_x_<8.00 && current_y_<1.50)
    {
    	goal_x= current_x_+2.00;
    	goal_y= 0.00;	
    }
    else if(current_x_>=8.00 && current_y_<7.30)
    {
    	goal_x= 9.40;
    	goal_y= current_y_ +2.00;
    }
    else if(current_y_>=7.30 && current_x_>-12.0)
    {
    	goal_x= current_x_-2.00;
    	goal_y= 8.70;
    } 
    ...
    */
    ...
    if (current_x_ <= 7.00 && current_y_ <= 2.34) {  // on the right side of the loop
        goal_x = 9.50;
        goal_y = 0.00;
        x_limit_top = 10.00;
        x_limit_bot = current_x_;
        y_limit_left = 0.37;
        y_limit_right = -0.66;
    } else if (current_x_ > 7.00 && current_y_ <= 6.25) {  // on the top side of the loop
        goal_x = 9.575;
        goal_y = 8.75;
        x_limit_top = 10.03;
        x_limit_bot = 9.12;
        y_limit_left = 9.20;
        y_limit_right = current_y_;
    } else if (current_x_ >= -11.5 && current_y_ > 6.25) {  // on the left side of the loop
        goal_x = -13.75;
        goal_y = 8.65;
        x_limit_top = current_x_;
        x_limit_bot = -14.00;
        y_limit_left = 9.15;
        y_limit_right = 8.15;
    } else if (current_x_ < -11.5 && current_y_ > 2.34) {  // on the bottom side of the loop
        goal_x = -13.79;
        goal_y = 0.00;
        x_limit_top = -13.32;
        x_limit_bot = -14.26;
        y_limit_left = current_y_;
        y_limit_right = -0.66;
    }
    ...
    while(count < max_rrt_iters_)
    {
        ...
        if(is_goal(new_node, goal_x, goal_y))
        {
            ...
            visualize_trackpoints(goal_way_point.position.x, goal_way_point.position.y, trackpoint[0], trackpoint[1]);
...
{
    std::uniform_real_distribution<>::param_type x_param(-14.00, 10.00);
    std::uniform_real_distribution<>::param_type y_param(9.50, -0.66);
...
std::vector<std::array<double, 2>> RRT::find_path(const std::vector<Node> &tree, const Node &latest_added_node)
{
    ...
    while(current_node.parent_index != -1)
    {
        ...
        std::cout<<"x"<<count<<":"<<current_node.x<<std::endl;
        std::cout<<"y"<<count<<":"<<current_node.y<<std::endl;
